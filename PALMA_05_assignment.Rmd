...
```{r}
 cp_profile <- function(explainer_fn, new_obs, `var`) {
  ...
  if (is.double(.data[[`var`]]) == TRUE) {
    return("Variable Is Not Formatted Correctly For Use.")
    break
  }
...
lending_integer <- select_if(lending_training, is.integer)
...
```{r}
cp_profile(rf_explain, obs2, "acc_now_delinq")
